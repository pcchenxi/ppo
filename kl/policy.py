...
class Policy(object):
    ...
    def __init__(self, obs_dim, act_dim, kl_targ, load_model=False):
        """
        Args:
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            if self.load_model:
                print ('Loading Model...')
                ckpt = tf.train.get_checkpoint_state('./model/rl/policy_model.cptk')
    ...
    def update(self, observes, actions, advantages, logger, episode, successrate):
        ...
        self.saver.save(self.sess, './model/rl/policy_model.cptk') 
        summary = tf.Summary()
        summary.value.add(tag='Loss/PolicyLoss', simple_value=float(loss))
        summary.value.add(tag='Loss/PolicyEntropy', simple_value=float(entropy))
        summary.value.add(tag='Loss/KL', simple_value=float(kl))
        ...
        summary.value.add(tag='Perf/Success Rate', simple_value=float(successrate))
