...
class NNValueFunction(object):
    ...
    def __init__(self, obs_dim, load_model=False):
        """
        Args:
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            out = tf.layers.dense(self.obs_ph, hid1_size, tf.tanh,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / self.obs_dim)), name="v_h1")
            out = tf.layers.dense(out, hid2_size, tf.tanh,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / hid1_size)), name="v_h2")
            out = tf.layers.dense(out, hid3_size, tf.tanh,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / hid2_size)), name="v_h3")
        ...
        if self.load_model:
            print ('Loading Model...')
            ckpt = tf.train.get_checkpoint_state('./model/rl/value_model.cptk')
    ...
    def fit(self, x, y, logger, episode):
        ...
        self.saver.save(self.sess, './model/rl/value_model.cptk') 
        # summary = tf.Summary()
